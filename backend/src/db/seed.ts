...
for (const name of deptNames) await db.insert(departments).values({ organizationId: organization.id, name }).onConflictDoNothing();
...
for(const p of people) await db.insert(employees).values({organizationId:organization.id,...p}).onConflictDoNothing({target:employees.email});
...
for(const [name,description,html] of templates) await db.insert(signatureTemplates).values({organizationId:organization.id,name,description,html,plainText:`{{display_name}} | {{job_title}} | {{company_name}} | {{email}}`,active:true}).onConflictDoNothing();
