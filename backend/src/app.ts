...
function gatewaySecretConfigured() {
  return Boolean(process.env.SIGNATURE_GATEWAY_API_KEY);
}

function requireDb(res: express.Response): Database | null {
  const d = getDb();
  if (!d) {
...
app.get('/api/health', async (_req, res) => {
  ...
  const base = {
    ...
    gatewayKeyConfigured: gatewaySecretConfigured(),
  ...
  try {
    ...
    return res.status(503).json({ ...base, ok: false, database: 'error' });
  }
});
...
app.post('/api/signatures/resolve', async (req, res) => {
  ...
  const input = z.object({
    ...
    messageType: z.enum(['new', 'reply', 'forward']).default('new'),
    bodyHtml: z.string().optional(),
  }).safeParse(req.body);
  if (!input.success) return res.status(400).json({ error: 'Invalid payload' });
  const d = requireDb(res); if (!d) return;
  ...
  try {
    ...
    const account = (await d.select().from(schema.emailAccounts).where(and(
      eq(schema.emailAccounts.organizationId, organizationId),
      eq(schema.emailAccounts.email, sender),
      eq(schema.emailAccounts.active, true),
    )).limit(1))[0];
    ...
    const employee = account.employeeId
      ? (await d.select().from(schema.employees).where(and(eq(schema.employees.id, account.employeeId), eq(schema.employees.organizationId, organizationId), eq(schema.employees.active, true))).limit(1))[0]
      : undefined;
    const department = employee?.departmentId
      ? (await d.select().from(schema.departments).where(and(eq(schema.departments.id, employee.departmentId), eq(schema.departments.organizationId, organizationId))).limit(1))[0]
      : undefined;
    const domain = account.domainId
      ? (await d.select().from(schema.domains).where(and(eq(schema.domains.id, account.domainId), eq(schema.domains.organizationId, organizationId), eq(schema.domains.active, true))).limit(1))[0]
      : undefined;
    const senderDomain = sender.split('@')[1]?.toLowerCase();
    const domainByEmail = !domain && senderDomain
      ? (await d.select().from(schema.domains).where(and(eq(schema.domains.organizationId, organizationId), eq(schema.domains.domain, senderDomain), eq(schema.domains.active, true))).limit(1))[0]
      : domain;

    let sig: any = null;
    let source = 'none';
    if (account.signatureId) {
      sig = (await d.select().from(schema.signatures).where(and(eq(schema.signatures.id, account.signatureId), eq(schema.signatures.organizationId, organizationId), eq(schema.signatures.published, true))).limit(1))[0];
      if (sig) source = 'account';
    }
    if (!sig && employee) {
      sig = (await d.select().from(schema.signatures).where(and(eq(schema.signatures.employeeId, employee.id), eq(schema.signatures.organizationId, organizationId), eq(schema.signatures.published, true))).orderBy(desc(schema.signatures.createdAt)).limit(1))[0];
      if (sig) source = 'employee';
    }
    if (!sig && department?.defaultSignatureId) {
      sig = (await d.select().from(schema.signatures).where(and(eq(schema.signatures.id, department.defaultSignatureId), eq(schema.signatures.organizationId, organizationId), eq(schema.signatures.published, true))).limit(1))[0];
      if (sig) source = 'department';
    }
    if (!sig && domainByEmail?.defaultSignatureId) {
      sig = (await d.select().from(schema.signatures).where(and(eq(schema.signatures.id, domainByEmail.defaultSignatureId), eq(schema.signatures.organizationId, organizationId), eq(schema.signatures.published, true))).limit(1))[0];
      if (sig) source = 'domain';
    }
    if (!sig) {
      sig = (await d.select().from(schema.signatures).where(and(eq(schema.signatures.organizationId, organizationId), eq(schema.signatures.published, true))).orderBy(desc(schema.signatures.createdAt)).limit(1))[0];
      if (sig) source = 'global';
    }
    ...
    if (!ver) return res.json({ inject: false, reason: 'version_missing' });
    ...
    const data: Record<string, string | undefined> = {
      ...
      display_name: employee ? `${employee.firstName} ${employee.lastName}` : sender,
    ...
    res.json({
      inject: true,
      signatureId: sig.id,
      version: sig.currentVersion,
      source,
      html: `${signatureMarkers.start}${html}${signatureMarkers.end}`,
...
app.post('/api/gateway/heartbeat', async (req, res) => {
  if (!gatewayAuth(req, res)) return;
  const d = requireDb(res); if (!d) return;
  try { res.json({ ok: true, time: new Date().toISOString() }); } catch { res.status(500).json({ error: 'heartbeat failed' }); }
});

app.post('/api/gateway/verify', async (req, res) => {
  if (!gatewayAuth(req, res)) return;
  res.json({ ok: true, service: 'signature-gateway', time: new Date().toISOString() });
});

app.post('/api/:resource', async (req, res) => {
  const table = resourceMap[req.params.resource];
  if (!table || req.params.resource === 'audit' || req.params.resource === 'gatewayKeys') return res.status(404).json({ error: 'Unknown or protected resource' });
  const d = requireDb(res); if (!d) return;
  try {
    const [row] = await d.insert(table).values(req.body).returning();
    if (req.body.organizationId) await recordAudit(d, req.body.organizationId, 'created', req.params.resource, row?.id, { fields: Object.keys(req.body) });
...
app.patch('/api/:resource/:id', async (req, res) => {
  ...
  try {
    ...
    if (row.organizationId) await recordAudit(d, row.organizationId, 'updated', req.params.resource, row.id, { fields: Object.keys(req.body) });
    res.json({ data: row });
