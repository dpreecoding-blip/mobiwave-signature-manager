...
app.get('/api/health', async (_req, res) => {
  if (!db) return res.json({ ok: true, service: 'mobiwave-signature-manager', database: 'not-configured', time: new Date().toISOString() });
  try { await db.execute('select 1'); res.json({ ok: true, service: 'mobiwave-signature-manager', database: 'connected', time: new Date().toISOString() }); }
  catch { res.status(503).json({ ok: false, service: 'mobiwave-signature-manager', database: 'error', time: new Date().toISOString() }); }
});
...
app.post('/api/signatures/resolve', async (req, res) => {
  ...
  try {
    ...
    let sig: any = null;
    if (account.signatureId) sig = (await d.select().from(schema.signatures).where(and(eq(schema.signatures.id, account.signatureId), eq(schema.signatures.organizationId, organizationId))).limit(1))[0];
    if (!sig && account.employeeId) sig = (await d.select().from(schema.signatures).where(and(eq(schema.signatures.employeeId, account.employeeId), eq(schema.signatures.organizationId, organizationId), eq(schema.signatures.published, true))).limit(1))[0];
    ...
    const data: Record<string,string|undefined> = { first_name: emp?.firstName, last_name: emp?.lastName, display_name: emp ? `${emp.firstName} ${emp.lastName}` : sender, job_title: emp?.jobTitle, department: dept?.name, email: sender, phone: emp?.phone, mobile: emp?.mobile, company_name: branding?.companyName || org?.name || 'MobiWave Innovations Ltd', website: branding?.website || 'https://mobiwave.co.ke', company_address: branding?.address, logo_url: branding?.logoUrl, disclaimer: branding?.disclaimer, ...((branding?.socialLinks || {}) as Record<string,string>) };
...
app.post('/api/:resource', async (req, res) => {
  ...
  const d = requireDb(res); if (!d) return;
  try { const [row] = await d.insert(table).values(req.body).returning(); res.status(201).json({ data: row }); } catch (e) { res.status(400).json({ error: 'create failed' }); }
});
...
app.patch('/api/:resource/:id', async (req, res) => {
  ...
  if (!table || req.params.resource === 'audit' || req.params.resource === 'gatewayKeys') return res.status(404).json({ error: 'Unknown or protected resource' });
  const d = requireDb(res); if (!d) return;
  try { const [row] = await d.update(table).set(cleanRecord(req.body)).where(eq(table.id, req.params.id)).returning(); row ? res.json({ data: row }) : res.status(404).json({ error: 'Not found' }); } catch { res.status(400).json({ error: 'update failed' }); }
});
